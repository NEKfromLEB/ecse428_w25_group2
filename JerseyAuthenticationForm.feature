...
So that I can verify its legitimacy before purchasing it from them.  
  ...
  Scenario: Successfully Submit a Jersey Authentication Form  
    ...
    And the jersey with brand "<brand>", model "<model>", serial number "<serialNumber>", year "<year>", and proof of authenticity "<proof>" shall exist in the system  
    Examples: 
      | brand  | model     | serialNumber | year | proof              | totalRequests |  
  ...
  Examples:  
    | brand  | model  | serialNumber | year | proof              |  
